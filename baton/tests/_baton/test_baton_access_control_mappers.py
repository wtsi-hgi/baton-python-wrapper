...
class _TestBatonAccessControlMapper(unittest.TestCase):
    ...
    def test_get_all_with_no_paths(self):
        self.assertEqual(self.mapper.get_all([]), set())

    def test_get_all_with_single_path(self):
        entity = self.create_irods_entity(_USERNAMES[0], self.access_controls)
        self.assertEqual(self.mapper.get_all(entity.path), set(self.access_controls))

    def test_get_all_with_multiple_path(self):
        entity_1 = self.create_irods_entity(_USERNAMES[0], self.access_controls)
        entity_2 = self.create_irods_entity(_USERNAMES[1], [self.access_control])
        entity_3 = self.create_irods_entity(_USERNAMES[2], [])
    ...
    def test_add_or_replace_single_access_control(self):
        entity = self.create_irods_entity(_USERNAMES[0], [self.access_control])
    ...
    def test_set_when_no_existing_access_controls(self):
        entity = self.create_irods_entity(_USERNAMES[0], ())
    ...
    def test_revoke_subset_of_access_controls(self):
        entity = self.create_irods_entity(_USERNAMES[0], self.access_controls + [self.access_control])
