...
class _TestBatonAccessControlMapper(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.access_controls = [AccessControl(_USERNAMES[0], zone, AccessControl.Level.WRITE),
    ...
    def test_add_with_invalid_path(self):
        self.assertRaises(FileNotFoundError, self.mapper.add, "/invalid", self.access_controls)

    def test_add_single_access_control(self):
        entity = self.create_irods_entity(NAMES[0], [self.access_control])
        self.mapper.add(entity.path, self.access_controls[0])
    ...
    def test_add_access_control_list(self):
        entity = self.create_irods_entity(NAMES[0], [self.access_control])
        self.mapper.add(entity.path, self.access_controls)
    ...
    def test_add_access_control_list_when_one_access_control_already_exists(self):
        entity = self.create_irods_entity(NAMES[0], [self.access_control])
        self.mapper.add(entity.path, self.access_controls + [self.access_control])
    ...
    def test_remove_with_invalid_path(self):
        self.assertRaises(FileNotFoundError, self.mapper.remove, "/invalid", self.access_controls)
    ...
    def test_remove_subset_of_access_controls(self):
        ...
        self.mapper.remove(entity.path, self.access_controls)
        print(self.mapper.get_all(entity.path))
